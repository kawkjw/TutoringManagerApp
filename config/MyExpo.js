...
export const registerForPushNotificationAsync = async () => {
    ...
    if (finalStatus !== "granted") {
        Alert.alert("경고", "알림 권한이 거부되어 있습니다.", [{ text: "확인", onPress: () => Linking.openSettings() }], {
            cancelable: false,
        });
        return null;
    }
...
export const pushNotificationsToPerson = async (sendFrom, uid, inputTitle, inputBody, inputData = {}) => {
    let messages = [];
    let tokens = [];

    await getDoc(doc(db, "users", uid))
        .then(async (user) => {
            const expoTokens = user.data().expoTokens;
